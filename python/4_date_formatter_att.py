...
class format_translator(gluon.Block):
    ...
    def forward(self, inputs, outputs):
        ...
        for i in range(self.out_seq_len):
            ...
            # Get score between (h_i, s_j)
            score_i = gemm2(enout, next_h.expand_dims(axis = 2))  # h = Batch * Timestep * (2 * hidden_dim), a = Batch * Timestep * r
...
if __name__ == '__main__':
    ...
    logger.info('Define model and trainer')
    ctx = mx.cpu()
    ...
    res = train(model, tr_data_iterator, te_data_iterator \
              ...
              , epochs= 300, ctx = ctx, output_file_name = 'format_translator_{}'.format(datetime.now().strftime('%Y%m%d')))
