...
class format_translator(gluon.Block):
    ...
    def forward(self, inputs, outputs):
        ...
        for i in range(self.out_seq_len):
            ...
            # Get additional score between (h_i, s_j)

            _n_h_s = nd.repeat(_n_h, repeats = enout.shape[1], axis = 1)
            _in_attn = nd.concat(enout, _n_h_s, dim = 2)
            #print('_in_attn shape = {}'.format(nd.array(_in_attn, ctx = self.ctx).shape))
...
if __name__ == '__main__':
    ...
    res = train(model, tr_data_iterator, te_data_iterator \
              ...
              , epochs= 300, ctx = ctx, output_file_name = '../models/additive_attention_{}'.format(datetime.now().strftime('%Y-%m-%d')))
